...
The three differ in useful ways. Rock and Whirlpool ride the **island chunk grid** and
...
- Behaviour is preserved for Rock and Whirlpool; the streaming logic lives in one
  ...
  dead.
- Inheritance was chosen as the idiomatic fit for this MonoBehaviour-heavy
  codebase; Fishing's extra complexity is absorbed by the
  optional tick hook without burdening every subclass.
...
- **Composition (`ChunkStreamer` helper driven by callbacks)**: more flexible and
  unit-testable in isolation, with heavier wiring than this codebase needs.
- **Base fits only the stateless spawners; Fishing stays separate**: simpler in
  the short term, but leaves the third copy of the streaming loop in place. The
  tick hook lets Fishing fit without contorting the others.
