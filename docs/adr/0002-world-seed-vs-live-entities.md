...
feature is reproduced from it. Fishing spots now derive their base seed from the
World Seed, read lazily after island generation resolves its possibly-randomized
seed.
...
1. *Isolated RNG now.* The spawner draws from its own `System.Random` seeded from
   ...
   cannot perturb the RNG of seeded systems. Encounters remain path-dependent.
2. *Per-entity persistence later.* In the save, each active enemy stores
   **position + current health + type**. Type is keyed by a new stable string
   `id` on `NightEnemyConfig`. Restoring partial health needs a new seam on
   `NightEnemyHealth`, since spawning currently forces max health.

## Consequences

- The world (islands, rocks, shops, fishing spots) is fully reproducible from a
  single integer in the save file.
- Enemy encounters are path-dependent, so the save carries the actual live
  enemies. Velocity and AI state (aggro, target, knockback) are treated as
  transient and re-derive on load. Position, health, and type are persisted.
- Enemy type identity is decoupled from asset names and array order, so renaming
  a config or reordering the spawner list keeps existing saves intact.
...
- **Reproducible enemy encounters from the seed**: spawns depend on the boat's
  trajectory and timing, so reproducing them would require simulation replay.
  Persisting active enemies is smaller and clearer.
- **Identify enemy type by asset name or list index**: both break old saves on a
  rename or reorder, with no compile-time error.
